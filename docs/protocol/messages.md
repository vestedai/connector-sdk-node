...
```proto
message Hello {
  string sdk_language = 1;  // "node", "python", "php", "go", …
...
|---|---|---|---|
| `sdk_language` | string | yes | Language identifier; used in admin UI and audit log. The Node.js SDK sends `"node"`. |
...
Node.js SDK example — the SDK constructs this automatically:
...
| `namespace` | string | Connector's declared namespace (e.g. `myapp`). All agent/tool keys must be prefixed with `<namespace>.`. |
...
The Node.js SDK auto-generates `input_schema_json` from the `static args` Zod schema via `zod-to-json-schema`, serialized to UTF-8 bytes.
