...
| `ns` | namespace string (e.g. `myapp`) | Hub checks that this matches `connectors.namespace`. |
...
token before the old token's revocation TTL expires. The Node.js SDK supervisor
