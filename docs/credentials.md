...
export class ErpCredentials implements UserCredentialHandler {
  ...
  async validate(
    ...
    const who = await this.erp.whoami(credential.username!, credential.password!);

    return who
      ? credentialOk({ account: who.login, role: who.role })
      : credentialFailed("ERP rejected those credentials.");
  }
...
```ts
async handle(args: Args, ctx: ToolContext) {
  const creds = ctx.credential();        // { username: "…", password: "…" }
